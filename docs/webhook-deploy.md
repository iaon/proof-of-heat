...
Скрипт `./run_app.sh` просто читает `VERSION`, печатает это значение в консоль и пересобирает сервис. Приложение читает тот же файл `VERSION` и показывает его в стартовом логе и в футере UI-страниц.

Если нужно изменить команды или рабочую директорию — правьте этот файл. Если нужно изменить отображаемую версию — правьте `VERSION`.
